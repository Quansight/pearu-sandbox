...
if __name__ == "__main__":
    ...
    for cls, kwargs in [
            ...
            (LigerLinearCrossEntropyLoss, args),
            *((nn.LinearCrossEntropyLoss,
               dict(options=nn.functional.LinearCrossEntropyOptions(grad_inplace=True, batch_chunk_size=1024 * k),
                    extra_label_fmt='{"gradinplace" if options.grad_inplace else ""}_batch_csz{options.batch_chunk_size}', **args)) for k in [1, 2]),
            (nn.LinearCrossEntropyLoss,
