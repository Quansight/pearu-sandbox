#  wget https://raw.githubusercontent.com/Quansight/pearu-sandbox/master/conda-envs/heavyai-dev.yaml
# conda env create  --file=heavyai-dev.yaml -n heavyai-dev

export USE_ENV=${USE_ENV:-heavyai-dev}
if hash conda; then
...
then
    ...
    echo "conda env create  --file=~/git/Quansight/pearu-sandbox/conda-envs/heavyai-dev.yaml -n $USE_ENV"
    exit 1
fi
...
To use different conda environment, run:
  ...
  <source activate-heavyai-dev.sh>
